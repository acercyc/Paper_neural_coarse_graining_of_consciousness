...
\rlstart{3}	
		...
		Our theory indicates that, to make information conscious, a sufficient and a necessary conditions need to be satisfied.
		...
		\begin{ants}
		    @ As described above, if information processing fails to meet either one of the two necessary conditions, information remains unconscious.
		    
		    @ First, considering processes which fail to meet the first condition. That is, a information process is not NTIC. We can see many processes in the neural network system fall in this category. 
		    
		    @ In this category, processes cannot form NTIC. The internal states of the processes are partially or fully determined by environment states, i.e. information is not closed. Reflective behaviours are one of the main subcategory.
		    
		    @Reflective behaviours usually are considered as automatic and non-conscious behavioural controls. \citep{casali2013theoretically}\needref{More referene}.
		    
		    @ In modern reinforcement learning framework, this usually is referred to policy-based agents. The agents constructs a policy function that maps sensory inputs to the action space \citep{sutton2018reinforcement}.
		    
		    @  Roughly speaking, a policy is a mapping from perceived states of the environment to actions to be taken when in those states. It corresponds to what in psychology would be called a set of stimulus–response rules or associations 
		    
		    @ The same principle can be applied to blindsight and procedure memory which are often considered as the classical cases of unconscious processing
		    
		    @ This classic patient study of D.F. who has apperceptive agnosia \citep{10.3389/fneur.2014.00255} also falls in this category. With online sensory inputs, the neural system can create functional processes without forming NTIC. 
		    
		    
		    @ Note that, when we prolong the temporal interval between stimulus and action, usually consciousness is necessary for learning associations. \needref{time}
		    
		    
		    @ Another case is that, processes are information closed but it is trivial. 
		    
		    
		    @ ============================================================================
		    
		    @ Even though the system can form NTIC processes in some certain coarse-grained levels, information processing outside these levels remains unconscious. 
		    
		    @ microscopic neural activities fall in this category. 
		    
		    @ When processes at macroscopic levels becomes informational closed, information from other levels is redundant. 
		    
		    @ Therefore, information from other levels does not influence the dynamic of this level. 
		...
		--- 
		procedure memory
		Reflective reaction, blindsight, 
		...
		\begin{aligned}
            \pi(A|S)\\
            {J_{t}(E \rightarrow Y )} & {:= I(Y_{t+1};E_{t}|Y_{t})} > 0
        \end{aligned}
		\end{equation}
		\needhelp{Math here}
		
		\begin{figure}
			\includegraphicsTodo[width=0.8\textwidth]{WritingMaterials/S_Y_A/S_Y_A_01.png}
