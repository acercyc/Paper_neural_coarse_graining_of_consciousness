...
\rlstart{4}        	As described above, if a process fails to fulfil either of the two conditions, information remains unconscious. First, We consider that a process fails to satisfy the first condition, i.e. the process fails to achieve NTIC. Many processes in the neural system fall into this category. 
            ...
            In the context of information processing in the neural system, regarding to perception, this indicates that the perceptual processes only passively responds to the sensory inputs. Regarding to action, this indicates that the process only produces reflective-like behaviour. Reflexive behaviours are commonly considered as automatic and non-conscious behavioural controls. \citep{casali2013theoretically}\needref{More referene}. We hypothesise that this is a natural consequence when a process fails to meet the first condition. 
            
            The same notion can be applied to blindsight and procedure memory \needref{blindsight}. Blindsight patients are able to track objects and avoid obstacles within the visual field without having any visual conscious experience. The direct stimulus-response mapping ($S\rightarrowR$) makes the internal neural circuit a non-NTIC process and results in unconscious processing. In the case of procedure memory, processes often relies on both the current environmental and system states to determine the next state of the processes\needref{Do I need ref??}. Therefore, they also fail to achieve high NTIC as well. 
            
             This is similar to the model-free policy-based agents in the reinforcement learning framework. The process of a system only maps the input state space to the action state space \needref{11}. The agent learns a policy $\pi$ which directly maps sensor values $S$ to output action $A$.
            % @ $\pi : S \times A$\\
    		\begin{equation}
...
\rlstart{3}        
            \begin{ants}
        	    ...
        	    @ This also explains why we are not aware of probabilistic inference performed by the neural system. 
        	    
        	    @ Previous studies have showed that the neural system can encode probability distribution and perform probabilistic inference in a near optimal way.
